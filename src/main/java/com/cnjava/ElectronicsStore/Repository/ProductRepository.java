...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(   "SELECT p "
    		+ "FROM Product p "
    		+ "WHERE p.Brand.BrandID = :brandid  and p.Category.CategoryID=:categoryid")
    ...
    @Query(   "SELECT p "
    		+ "FROM Product p "
    		+ "WHERE p.Category.CategoryID = :categoryid and p.Brand.BrandID=:brandid")
    ...
    @Query(   "SELECT COUNT(p) "
    		+ "FROM Product p "
    		+ "WHERE p.Category.CategoryName LIKE %:keyword% OR p.Brand.BrandName LIKE %:keyword% OR p.ProductName LIKE %:keyword%")
